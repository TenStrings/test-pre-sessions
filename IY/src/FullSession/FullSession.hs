...
recv' :: (Pickup ss n (Recv v u), Update ss n u ss', IsEnded ss F)
      => Channel t n -> (v -> Bool) -> Session t ss ss' v
recv' (C n) p = Session (\ss -> case pickup ss n of (Recv inp u) -> inp >>= \x -> if p x then return (update ss n u, x) else error "invalid property")
...
class Par ss tt' tt where -- | ss tt' -> tt, ss tt -> tt', tt tt' -> ss where
   split :: tt -> IO (ss, tt')
...
class Par' t ss tt' tt where -- | t ss tt' -> tt, t ss tt-> tt', t tt tt' -> ss where
