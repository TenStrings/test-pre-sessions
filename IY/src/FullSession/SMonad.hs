{-# LANGUAGE TypeOperators, KindSignatures, ScopedTypeVariables #-}
...
-- 
{-@ data Session t ss tt a = Session { session :: ss -> IO (tt, a) } @-}
...
(Session m) >>>= f = Session (m >=> (\ (tt, x) -> session (f x) tt))

(>>>) :: Session t ss tt a -> Session t tt uu b -> Session t ss uu b
(Session m) >>> (Session n) = Session (m >=> (\ (tt, _) -> n tt))
